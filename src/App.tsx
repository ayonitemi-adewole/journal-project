    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          ...
          <section className="panel calendar-panel"><div className="panel-heading"><div><h2>Daily performance</h2><p>Daily P&L, trade count, and win rate</p></div><div className="calendar-controls"><strong className={calendarMonthProfit >= 0 ? 'positive' : 'negative'}>{money(calendarMonthProfit)} month</strong><button className="icon-button" onClick={() => setCalendarMonth(new Date(calendarDate.getFullYear(), calendarDate.getMonth() - 1, 1).toISOString().slice(0, 7))}>&lsaquo;</button><strong>{calendarDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</strong><button className="icon-button" onClick={() => setCalendarMonth(new Date(calendarDate.getFullYear(), calendarDate.getMonth() + 1, 1).toISOString().slice(0, 7))}>&rsaquo;</button></div></div><div className="calendar-weekdays">{['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{calendarWeeks.map((week, weekIndex) => <div className="calendar-week-row" key={`week-${weekIndex}`}>{week.days.map((item, dayIndex) => item ? <button key={item.day} className={`calendar-day ${item.stats ? (item.stats.profit >= 0 ? 'profit-day' : 'loss-day') : ''}`} onClick={() => { if (item.stats) { setPeriod('Custom'); setCustomStart(item.day); setCustomEnd(item.day) } }}><strong>{Number(item.day.slice(-2))}</strong>{item.stats ? <><b>{money(item.stats.profit)}</b><small>{item.stats.trades} trade{item.stats.trades === 1 ? '' : 's'} · {((item.stats.wins / item.stats.trades) * 100).toFixed(0)}%</small></> : <small>No trades</small>}</button> : <span className="calendar-empty" key={`empty-${weekIndex}-${dayIndex}`} />)}<div className="calendar-week-total"><span>Week {weekIndex + 1}</span><strong className={week.profit >= 0 ? 'positive' : 'negative'}>{money(week.profit)}</strong><small>{week.trades} trade{week.trades === 1 ? '' : 's'}</small></div></div>)}</div></section>
